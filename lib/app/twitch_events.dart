...
extension ScopeSubscription on TwitchAppScope {
...
class TwitchEvents {
  ...
  static Future<TwitchEvents> factory({
    ...
    for (final eventScope
        in appInfo.scope.where((e) => e.scopeType == AppScopeType.events)) {
  ...
  final onRewardRedeemed =
      TwitchListener<void Function(TwitchRewardRedemption reward)>();
  ...
  Future<void> _sendPostSubscribtionRequest(TwitchAppScope scope) async {
    _logger.info('Subscribing to Twitch events...');

    if (scope.scopeType != AppScopeType.events) {
